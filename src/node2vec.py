...
class Graph():
	...
	def node2vec_walk(self, walk_length, start_node):
		...
		while len(walk) < walk_length:
			...
			if len(cur_nbrs) > 0:  # 如果有邻居节点
				if len(walk) == 1:  # start node位置
	...
	def simulate_walks(self, num_walks, walk_length):
		...
		for walk_iter in range(num_walks):  # 每一次循环为所有节点生成walk，一共生成num_walks次
	...
	def get_alias_edge(self, src, dst):  # src(t): edge[0], dst(v): edge[1]
		...
		unnormalized_probs = []
		# 核心算法
		# print("src:", src, " dist:", dst, "src_type:", type(src))
