	...
	def get_alias_edge_attributed(self, src, dst):
		...
		for dst_nbr in sorted(G.neighbors(dst)):
			if dst_nbr == src:  # x = t
				...
				bias_vx = (1/beta) * (1 - G[dst][dst_nbr]['distance']/dv)
				tx_HasEdge = G.has_edge(src, dst_nbr)  # 判断t-x是否有边
				if tx_HasEdge:  # 如果t-x有边，判断是否为inward
					bias_tx = beta * (1 - G[src][dst_nbr]['distance']/dt)
					if abs(G[src][dst_nbr]['distance']) <= dt:  # t-x tight
						alpha = 1 # INWARD
					elif abs(G[dst][dst_nbr]['distance']) <= dv:  # v-x tight
						alpha = 1 / q + (1 - 1 / q) * (dt / G[src][dst_nbr]['distance'])  # SPECIAL
					else:
						alpha = min(1, 1 / q)  # NOISY
				else:  # t-x 无边
					if abs(G[dst][dst_nbr]['distance']) <= dv:  # v-x tight
						alpha = 1/q  # OUT
					else:  # v-x loose
						alpha = min(1, 1 / q)  # NOISY

			unnormalized_probs.append(G[dst][dst_nbr]['weight'] * ( alpha + bias_vx + bias_tx))
