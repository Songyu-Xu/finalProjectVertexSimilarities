...
class Graph():
	...
	def preprocess_distance(self, sigma=1):
		...
		self.feat_dict_ss = feat_dict_ss  # 存储归一化后的信息
		# print(feat_dict_ss)
		...
		# 计算所有节点的threshold
		for node in G.nodes():
			threshold = self.calculate_threshold(node)
			G.nodes[node]["threshold"] = threshold

		return 0

	def calculate_threshold(self, v):
		"""
		Calculate the threshold given the centre vertex
		"""
		G = self.G
		feat_dict_ss = self.feat_dict_ss
		gamma = self.gamma
		...
		if len(nbrs) > 0:
			for v_nbr in nbrs:
				# TODO: 分出去
				distance = self.calculate_attr_diff(v, v_nbr)
	...
	def get_alias_edge_attributed(self, src, dst):
		...
		# t--v--x-
		dt = G.nodes[src]["threshold"]
		dv = G.nodes[dst]["threshold"]
		bias = 0
		# 核心算法
		for dst_nbr in sorted(G.neighbors(dst)):
			if dst_nbr == src:  # x = t
				alpha = 1/p  # RETURN NODE
			else:
				# bias = (1/beta) * (1 - self.calculate_attr_diff(src,dst_nbr)/dv)
				tx_HasEdge = G.has_edge(src, dst_nbr)  # 判断t-x是否有边
				if tx_HasEdge:  # 如果t-x有边，判断是否为inward
					# bias_tx = beta * (1 - G[src][dst_nbr]['distance']/dt)
					if abs(self.calculate_attr_diff(src,dst_nbr)) <= dt:  # t-x tight
						# alpha = 1 # INWARD
						alpha = 1+beta  # INWARD
					elif abs(self.calculate_attr_diff(dst,dst_nbr)) <= dv:  # v-x tight
						# alpha = 1 / q + (1 - 1 / q) * (dt / self.calculate_attr_diff(src,dst_nbr))  # SPECIAL
						alpha = 1 / q + (beta - 1 / q) * (dt / self.calculate_attr_diff(src, dst_nbr))  # SPECIAL
					else:
						alpha = min(1, 1 / q)  # NOISY
				else:  # t-x 无边
					if abs(self.calculate_attr_diff(dst,dst_nbr)) <= dv:  # v-x tight
						alpha = 1/q  # OUT
					else:  # v-x loose
						alpha = min(1, 1 / q)  # NOISY

			unnormalized_probs.append(G[dst][dst_nbr]['weight'] * ( alpha + 3*bias))
	...
	# 计算节点u,v之间属性差异度
	def calculate_attr_diff(self, u, v):
		feat_dict_ss = self.feat_dict_ss
		u_feat = feat_dict_ss.get(u)
		v_feat = feat_dict_ss.get(v)
		diff = np.exp(-(np.linalg.norm(u_feat - v_feat)) ** 2)
		return diff
