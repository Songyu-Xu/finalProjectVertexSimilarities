...
def main(args):
    compact_matrix = np.zeros((round(((args.q_max-args.q_min)/args.q_step) + 1),
                               round((args.p_max-args.p_min)/args.p_step) + 1))
    print(compact_matrix.shape)
    p = args.p_min
    q = args.q_min
    iter_p = 0
    iter_q = 0
    iterp_max = round((args.p_max-args.p_min)/args.p_step)
    iterq_max = round((args.q_max-args.q_min)/args.q_step)
    # walk_length 与 num_walks 循环
    while iter_p <= iterp_max:
        args.p = p
        q = args.q_min
        iter_q = 0
        while iter_q <= iterq_max:
            args.q = q
            print("\n******************************\n",
                  "p = ", args.p, ", q = ", args.q)
            q = round(q + args.q_step, 1)
            ...
            compact_matrix[iter_q, iter_p] = format(graph_average_compact, '.2f')
            iter_q += 1
        p = round(p + args.p_step, 1)
        iter_p += 1

    # 画热力图
    draw_heatmap(args, compact_matrix, iterp_max, iterq_max)
